...
(() => {
  angular.module('meanbaseApp')
    .directive('chooseImage', ($rootScope, $timeout) => new ChooseImageDirective($rootScope, $timeout))

  class ChooseImageDirective {
    ...
    compile(element, attrs) {
      return {
        pre: function preLink(scope, element, attrs, controller) {
          if(!self.$rootScope.isLoggedIn) { return false; }
          ...
          function getBelongsTo() {
            if(attrs.belongsTo) {
              ...
              scope.belongsTo = self.$rootScope.page.images;
              key = attrs.property;
              if(!scope.belongsTo[key]) {
                scope.belongsTo[key] = {};
              }
